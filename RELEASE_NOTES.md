# Binaural Companion v0.1.0

First public Android release.

## Highlights

- Adds a quiet, locally generated stereo layer while Spotify or another media app continues playing.
- Includes nine evidence-labeled modes: four everyday binaural presets, neutral control, Binaural 40, MIT/Martorell 1 ms tone pips, human 40 Hz AM flutter, and human 40 Hz clicks.
- Adds an optional capped Music assist that detects concurrent playback and smoothly raises only the local stream by 25%, without capturing or analyzing song audio.
- Supports steady differences or gradual 0-40 Hz glides around a carrier-centered stereo pair.
- Offers an optional Gateway-inspired preparation using ordinary intention, breathing, and relaxation practices.
- Stores optional before/after calm, focus, and energy ratings only on the device.
- Uses no microphone, network, analytics, account, or audio-capture permission.

## Installation

1. Download `binaural-companion-v0.1.0.apk` from the release assets.
2. Allow your browser or file manager to install apps from this source when Android asks.
3. Install the APK and connect stereo headphones.
4. Begin at a low level—especially with tone pips or clicks—and stop if you feel discomfort.

## Important boundary

This is an independent educational and wellness prototype. The MIT/Martorell pathology and memory results were in mice; human ASSR is a measured neural response, not proof of treatment or broad cognitive enhancement. The app is not affiliated with Spotify, MIT, the Monroe Institute, the CIA, or the owners of the Hemi-Sync trademark. It is not a medical device and does not provide medical, paranormal, or guaranteed cognitive effects.
